...
help_message = ("To change the language, enter the /start command, choose a language and enter the name of a "
                "location. Please wait, if the bot does not respond, there may be short-term connection "
                "delays.\n\nЧтобы поменять язык, введите команду /start, выберите язык и введите название локации. "
                "Пожалуйста, подождите, если бот не отвечает, возможны кратковременные задержки подключения.")
